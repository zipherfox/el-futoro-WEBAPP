...
tu_data = {}
for faculty_dict3 in tuweight2.tuweight:
    ...
    if faculty in kmitl_data and isinstance(kmitl_data[faculty], dict):
        ...
        while True:
            ...
            if sub_major in kmitl_data[faculty]:
                break
            print("Invalid sub-major. Please choose from the list.")
    if faculty in tu_data and isinstance(tu_data[faculty], dict):
        print("\nAvailable sub-majors:", ", ".join(tu_data[faculty].keys()))
        while True:
            sub_major = input("Enter your sub-major: ").strip()
            if sub_major in tu_data[faculty]:
                break
            print("Invalid sub-major. Please choose from the list.")
